...
using DiskUtility.Views.Backdrop;
using DiskUtility.Views.Pages;
using DiskUtility.WindowsAPI.ComTypes;
using DiskUtility.WindowsAPI.PInvoke.Comctl32;
using DiskUtility.WindowsAPI.PInvoke.Dwmapi;
using DiskUtility.WindowsAPI.PInvoke.Gdi32;
using DiskUtility.WindowsAPI.PInvoke.Shell32;
using DiskUtility.WindowsAPI.PInvoke.User32;
using DiskUtility.WindowsAPI.PInvoke.Uxtheme;
...
namespace DiskUtility.Views.Windows
{
    public class MainWindow : Form
    {
        ...
        public async Task<ContentDialogResult> ShowDialogAsync(ContentDialog contentDialog)
        {
            ...
            if (!isDialogOpening && contentDialog is not null && Content is not null)
            {
                ...
                {
                    LogService.WriteLog(EventLevel.Error, nameof(DiskUtility), nameof(MainWindow), nameof(ShowDialogAsync), 1, e);
                }
        ...
        public async Task ShowNotificationAsync(TeachingTip teachingTip, int duration = 2000)
        {
            if (teachingTip is not null && Content is Page page && page.Content is Grid grid)
            {
                ...
                {
                    LogService.WriteLog(EventLevel.Error, nameof(DiskUtility), nameof(MainWindow), nameof(ShowNotificationAsync), 1, e);
                }
            }
        }
