...
namespace DiskUtility.WindowsAPI.PInvoke.User32
{
    ...
    public enum WindowExStyle : uint
    {
        ...
        /// <summary>
        /// 使用双缓冲按从下到上绘制顺序绘制窗口的所有后代。 从下到上绘制顺序允许后代窗口具有半透明 (alpha) 和透明度 (颜色键) 效果，但前提是后代窗口还设置了 WS_EX_TRANSPARENT 位。 双缓冲允许不闪烁地绘制窗口及其后代。 如果窗口的 类样式 为 CS_OWNDC 或 CS_CLASSDC，则不能使用此样式。
        /// Windows 2000：不支持此样式。
        /// </remarks>
        WS_EX_COMPOSITED = 0x02000000,

        /// <summary>
        /// 窗口的标题栏包含问号。 当用户单击问号时，光标将更改为带有指针的问号。 如果用户单击子窗口，子窗口将收到 WindowMessage.WM_HELP 消息。 子窗口应将消息传递给父窗口过程，该过程应使用 HELP_WM_HELP 命令调用 WinHelp 函数。 帮助应用程序会显示一个弹出窗口，该窗口通常包含子窗口的帮助。
        /// WS_EX_CONTEXTHELP 不能与 WindowStyles.WS_MAXIMIZEBOX 或 WindowStyles.WS_MINIMIZEBOX 样式一起使用
        /// </summary>
        ...
        /// <summary>
        /// 窗口具有双边框;可以选择使用标题栏创建窗口，方法是在 dwStyle 参数中指定 WindowStyles.WS_CAPTION 样式。
        /// </summary>
        ...
        /// 若要激活窗口，请使用 SetActiveWindow 或 User32Library.SetForegroundWindow 函数。
        ...
        /// <summary>
        /// 使用此样式创建的子窗口不会在创建或销毁时将 WindowMessage.WM_PARENTNOTIFY 消息发送到其父窗口。
        /// </summary>
